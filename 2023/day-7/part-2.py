...
class Hand:
    def __init__(self, hand: str, bid: str):
        self.hand_str = hand
        self.bid = int(bid)
        self.counts, self.wildcards = self.calculate_counts(hand)
        self.type = self.calculate_type()
    

    def calculate_counts(self, hand: str):
        count_map, wildcards = {}, 0
        max_card, max_count = None, float('-inf')
        ...
        for card in hand:
            ...
            if count_map[card] > max_count:
                max_count = count_map[card]
                max_card = card
        
        if 0 < wildcards < 5:
            count_map[max_card] += wildcards
        
        if wildcards==5:
            count_map['J'] = wildcards

        return count_map, wildcards
